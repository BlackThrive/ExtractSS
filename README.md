...
## Example pipeline and usage <a name="examples"></a>

The aim of this section is to outline how to quickly get started using extractSS. For more details on the functions in extract_ss and their parameters, see the extract_ss Wiki.

The main workhorse of extract_ss is the function 'extract_ss_data'. The simplest usage requires only the argument ‘coord_list’, which is the list of areas for which to acquire data. By default, the function will extract for the most recent 12 months. So, if the user wishes to extract data for all the areas available, the call would be:
```R
